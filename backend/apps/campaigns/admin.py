...
class CampaignAdmin(AuditedAdminMixin, admin.ModelAdmin):
    ...
    def launch_campaign(self, request, queryset):
        ...
        for campaign in queryset:
            self._launch_one(request, client, campaign)

    def _launch_one(self, request, client, campaign):
        ...
        if not campaign.target_department:
            self.message_user(request, f"{campaign} has no target department — skipped.", level=messages.ERROR)
            return
